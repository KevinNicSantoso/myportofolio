...
1. Django's ModelForm is used to generate the form fields automatically from models.py and it inherits all validation rules (max_length, choices), while form.is_valid() and form.save() handle conversion, validation, and database saving without duplicating HTML inputs which keeps the form permanently in sync with the database schema, eliminates repetitive boilerplate, and prevents human errors such as mismatched field names. The {% csrf_token %} tag is mandatory in these POST forms because it renders a secret token that Django compares against the user's session when the form is submitted, blocking Cross-Site Request Forgery attacks in which a malicious site tricks the browser into sending forged requests (for example, deleting an experience entry).
